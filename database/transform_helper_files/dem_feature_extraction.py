...
def get_whitebox_tools() -> whitebox.WhiteboxTools:
...
def get_slope(dem_path: str, output_path: str, xy_coords: np.ndarray) -> np.ndarray:
    assert '.tif' in output_path, "Output file extention must be a .tif file"
    output_path = os.path.abspath(output_path)
    if not os.path.exists(output_path):
        print(f'Creating slope DEM at: {output_path}')
        wbt = get_whitebox_tools()
        wbt.slope(dem_path, output_path, units='degrees')
    slope = extract_values_from_dem(output_path, xy_coords)
    return slope
